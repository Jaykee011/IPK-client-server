...
/*
	...
	Při chybě ukončí server a vrátí -1
*/
int socketInit()
{
	int serverSocket;
...
/*
	napojení socketu na port a adresu
*/
...
/*
	vytvoří náležitou HTTP zprávu a odešle ji jako odpověď klientovi
*/
void sendResponse(string result, int socket, struct parsedMsg* request)
{
	if (result == "Not a file." || result == "Not a directory." || result == "Unknown error." || result == "Directory not empty.")
...
/*
	pomocí regulárních výrazů zanalyzuje request od klienta. Náležitě nastaví strukturu request.
*/
...
/*
	provede příkazvyžádaný klientem, ve stringu result vrací případné chybové hlášení/soubor
*/
int execute(struct parsedMsg request, string root, string *result)
{
	//cout << request.user << "\n\n";
	//cout << root + "/" + request.user + "/" << "\n";
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	/*
		Nastavení správné adresy serveru
	*/
	...
	*/
	...
	while(1) //smyčka přijímání zpráv
	{
