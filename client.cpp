...
int parseMsg(string message, struct parsedMsg* request, string* result)
{
	...
	if (request->HTTP == "404 Not found" || request->HTTP == "400 Bad Request")
	{
		...
		if (request->trailer == "Not a file.")
			exit(NOTAFILE);
		else if (request->trailer == "Not a directory.")
			exit(NOTADIR);
		else if (request->trailer == "File not found.")
			exit(FILENOTFOUND);
		else if (request->trailer == "User Account Not Found.")
			exit(USERNOTFOUND);
		else if (request->trailer == "Directory not found.")
			exit(DIRNOTFOUND);
		else if (request->trailer == "Directory not empty.")
			exit(DIRNOTEMPTY);
		exit(ERROR);
	}
